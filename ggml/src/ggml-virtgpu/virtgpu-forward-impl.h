...
#define REMOTE_CALL(gpu_dev_name, encoder_name, decoder_name, ret_name)                                           \
    do {                                                                                                          \
        uint32_t _temp_ret = (gpu_dev_name)->ops->remote_call((gpu_dev_name), (encoder_name), &(decoder_name), 0, NULL); \
        if (!(decoder_name)) {                                                                                    \
            printf("FATAL: %s: failed to kick the remote call\n", __func__);                                   \
            fflush(stdout);                                                                                      \
            exit(1);                                                                                             \
        }                                                                                                         \
        if (_temp_ret < APIR_FORWARD_BASE_INDEX) {                                                                \
            printf("FATAL: %s: failed to forward the API call: %s: code %u\n", __func__,                       \
                   apir_forward_error((ApirForwardReturnCode)_temp_ret), _temp_ret);                             \
            fflush(stdout);                                                                                        \
            exit(1);                                                                                               \
        }                                                                                                         \
